...
plot_bars_wo_pval <- function(vector_groups,vector_group_names,phenotype_n){
  for(i in c(1:length(vector_groups))){
    if(i==1){df <- data.frame()
    table <- c()}
    pa <- phenotype_analysis(get(vector_groups[i])[,phenotype_n])
  ...
  df_plot <- df
  p<- ggplot(data = df_plot, aes(x = factor(group,levels = vector_group_names), y = penetrance)) +
    geom_bar(stat="identity", colour="black", fill="grey") + labs(title=phenotypes[phenotype_n-6]) +  
    geom_errorbar(aes(ymin = lower*100, ymax = upper*100), width = .2) + 
    theme(axis.text = element_text(size = 10), 
...
```{r fig.width = 9, fig.height = 4.5, fig.align = "center", out.width = '60%'}
...
```{r fig.width = 6, fig.align = "center", out.width = '40%'}
barplot(penetrance, ylim = c(0,100), ylab = "Penetrance of symptoms (%)", las = 1, names.arg = c("RD","OBE","DD","PD","CI","REP","REN","LIV","HRT"), main = "Penetrance of BBS symptoms",
        xlab = paste("chisq.test p-value = ", format(chisq.test(penetrance_table)$p.value,digits = 4)))
```
...
```{r fig.width = 6, fig.align = "center", out.width = '40%'}
barplot(penetrance, ylim = c(0,100), ylab = "Penetrance of symptoms (%)", names.arg = c("RD","OBE","PD","CI","REN"), main = "Penetrance of BBS symptoms", las = 1,
        xlab = paste("chisq.test p-value = ",format(chisq.test(penetrance_table)$p.value,digits = 4)))
```
...
boxplot(subset_sscore$si~factor(subset_sscore$ggroup,levels = c("bbs3","bbsome","chaps","other")), las = 2, ylab = "Syndromic score", xlab = "")
...
## test of chaperonins
boxplot(subset_chaperonins$si~droplevels(subset_chaperonins$gene), las = 1, ylab = "Syndromic score", xlab = "")
...
boxplot(subset_sscore_bbsome_wobbs18$si~subset_sscore_bbsome_wobbs18$gene, las = 1, ylab = "Syndromic score", xlab = "")
...
```{r fig.width= 4, fig.align = "center", out.width='35%'}
...
boxplot(subset_sscore$si~subset_sscore$muttype, las = 1, ylab = "Syndromic score", xlab = "")
...
boxplot(subset_sscore_cLOF_bbsome_wobbs18$si~subset_sscore_cLOF_bbsome_wobbs18$gene, las = 1, ylab = "Syndromic score", xlab = "")
...
```{r fig.width=4, fig.height=4, fig.align = "center", out.width="35%"}
...
```{r fig.height=8.5, fig.align = "center", fig.width=4.5, message=FALSE, warning=FALSE}
...
```{r fig.width=4.5, fig.height=8.5, fig.align = "center"}
...
fisherize_one_sided <- function(subset1,subset2,phenotype_n,all_or_cLOF,alternative){
  ...
  if(all_or_cLOF=="all"){
    print(plot_bars_wo_pval(c(deparse(substitute(subset1)),deparse(substitute(subset2))),c(deparse(substitute(subset1)),deparse(substitute(subset2))),phenotype_n
  ))
...
### Brinckman, 2013 
CI
Fisher’s exact test BBS1 vs BBS10 patients 
```{r fig.width=3, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
fisherize_one_sided(subset_bbs1,subset_bbs10,10,"all","greater")

```

### Abu Safieh, 2010
Fisher’s exact test BBS3 vs other canonical genes
CI, REP

```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
Fisher’s exact test BBS1 vs chaperonins
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
Fisher’s exact test BBS12 vs BBS1 and BBS10 combined
CI
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
CI
Fisher’s exact test BBS1 vs other canonical BBS genes
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
Fisher’s exact test BBS12 vs other canonical BBS genes
CI
...
Fisher’s exact test BBS10 vs other canonical BBS genes
REN
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
### Esposito, 2017
RD
Fisher’s exact test BBS10 vs BBS1 and BBS2 patients
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
### Forsythe, 2015
REN, LIV
Fisher’s exact test BBS10 vs BBS1 
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
REN, LIV
Fisher’s exact test BBS1 and BBS10 MS/MS vs. TR/TR 
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
### Forsythe, 2017
REN
Fisher’s exact test BBS2, BBS10 and BBS12 vs other canonical
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
REN
Fisher’s exact test all canonical MS/MS vs. TR/TR
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
REN
Fisher’s exact test BBS1 M390R hom vs. BBS1 MS/TR+TR/TR
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
REN, CI, OBE, REP
Fisher’s exact test BBS1 vs BBS2 and BBS10
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
### Kerr, 2015 
CI
Fisher’s exact test BBS1 vs BBS10 vs other canonical (three groups) 
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
...
### Schaefer, 2011a 
PD, REN
Fisher’s exact test BBS16 vs all canonical
```{r fig.width=6, fig.height=3, out.extra='style="float:left; padding:10px; height:82px; width:220px"'}
