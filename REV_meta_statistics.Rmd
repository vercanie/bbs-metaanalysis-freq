...
update8forR <- read_delim("G:/48_lab/Project BBSome in T cells/!Manuscript META in preparation/!!Revision Human Mutation/update8forR.csv", 
...
plot_bars<- function(vector_groups,vector_group_names,phenotype_n){
  ...
  geom_bar(stat="identity", colour="black", fill="grey") + labs(title=phenotypes[phenotype_n-6]) + xlab(paste("p-value = ",format.pval(p_val,digits = 4))) +  
...
```{r}
sidak <- as.data.frame(matrix(c("#","*","**","***","****","< 0.1","< 0.05","< 0.1","< 0.001","< 0.0001","< 0.033","< 0.017", "< 0.003","<3.3e-4","<3.3e-5"),ncol = 3,byrow = FALSE))
names(sidak) = c("symbol","orig p-value","Sidak p-value")
kable(sidak, format= "html") %>%
  kable_styling(full_width = F, font_size = 9,bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
sidak3 <- c(0.033,0.017,0.003,3.3e-4,3.3e-5)
...
```{r}
sidak <- as.data.frame(matrix(c("#","*","**","***","****","< 0.1","< 0.05","< 0.1","< 0.001","< 0.0001","< 1.49e-2","< 7.3e-3", "< 1.43e-3","< 1.43e-4","< 1.43e-5"),ncol = 3,byrow = FALSE))
names(sidak) = c("symbol","orig p-value","Sidak p-value")
kable(sidak, format = "html") %>%
  kable_styling(full_width = F, font_size = 9,bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
sidak7 <- c(1.49e-2, 7.3e-3, 1.43e-3, 1.43e-4, 1.43e-5)
...
```{r}
one_vs_all_other(subset_bbs7, subset_bbsome,10,c(0,0,0,0,0))
one_vs_all_other(subset(subset_bbs7, subset_bbs7[16]=="cLOF"), 
                 subset(subset_bbsome, subset_bbsome[16]=="cLOF"),
                 10,c(0,0,0,0,0))
...
```{r}
one_vs_all_other(subset_bbs2, subset_bbsome,13,c(0,0,0,0,0))
one_vs_all_other(subset(subset_bbs2, subset_bbs2[16]=="cLOF"), 
                 subset(subset_bbsome, subset_bbsome[16]=="cLOF"),
                 13,c(0,0,0,0,0))
```
...
```{r}
one_vs_all_other(subset_bbs5, subset_bbsome,14,c(0,0,0,0,0))
one_vs_all_other(subset(subset_bbs5, subset_bbs5[16]=="cLOF"), 
                 subset(subset_bbsome, subset_bbsome[16]=="cLOF"),
                 14,c(0,0,0,0,0))
```
...
```{r}
one_vs_all_other(subset_bbs4, subset_bbsome,14,c(0,0,0,0,0))
one_vs_all_other(subset(subset_bbs4, subset_bbs4[16]=="cLOF"), 
                 subset(subset_bbsome, subset_bbsome[16]=="cLOF"),
                 14,c(0,0,0,0,0))
```
